...
def get_equipment_status(equipment: str): 
    ...
    if dataset: 
        ...
        vibration = float(sensor_features.get('sensor_00', sensor_features.get('vibration_velocity', 
sensor_features.get('vibration', 1.01)))) 
        temperature = float(sensor_features.get('sensor_01', sensor_features.get('bearing_temp', 
sensor_features.get('temperature', 50.3)))) 
        pressure = float(sensor_features.get('sensor_02', sensor_features.get('inlet_pressure', 
sensor_features.get('pressure', 48.3)))) 
        efficiency = float(sensor_features.get('efficiency', sensor_features.get('calculated_efficiency', 
81.5))) 
    else: 
        predicted_rul = 338.3 
        vibration = 1.01 
...
def simulate_equipment(equipment: str, req: SimulationRequest): 
    ...
    simulated_efficiency = max(0.0, min(100.0, 95.0 - (req.vibration * 4) - ((req.temperature - 40) * 
0.3))) 
