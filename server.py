...
def simulate_equipment(equipment: str, req: SimulationRequest): 
    ...
    # Extract default baseline feature row from dataset if available
    baseline_features = {}
    if dataset:
        baseline_features = {k: v for k, v in dataset[0].items() if k not in ['actual_rul', 'predicted_rul_hours', 'rul']}

    # Assign fallback values if request params are None
    ...
    # Overwrite baseline features with user simulated values
    baseline_features.update({
        "vibration_velocity": vib_val, "sensor_00": vib_val, "vibration": vib_val,
        "bearing_temp": temp_val, "sensor_01": temp_val, "temperature": temp_val,
        "inlet_pressure": press_val, "sensor_02": press_val, "pressure": press_val
    })
    ...
    # Algorithmic fallback if model execution failed or pipeline was not loaded
